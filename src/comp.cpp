...
void Comp::createVisual(Microsoft::WRL::ComPtr<IDCompositionVisual2> &visual, const Microsoft::WRL::ComPtr<IDCompositionVirtualSurface> &content)
{
...
{
    HRESULT hr = this->device->CreateVirtualSurface(this->currentSize.width, this->currentSize.height, DefaultFormat, DefaultAlpha, &surface);
    ThrowOnError(hr);
}
void Comp::createVisualSurfacePair(Microsoft::WRL::ComPtr<IDCompositionVisual2> &visual, Microsoft::WRL::ComPtr<IDCompositionVirtualSurface> &surface)
{
    this->createVisual(visual);
...
inline void makeRelation(const Microsoft::WRL::ComPtr<IDCompositionVisual2> &parent, const Microsoft::WRL::ComPtr<IDCompositionVisual2> &child)
{
    HRESULT hr = parent->AddVisual(child.Get(), true, nullptr);
    ThrowOnError(hr);
}
inline void setEffect(const Microsoft::WRL::ComPtr<IDCompositionVisual2> &target, const Microsoft::WRL::ComPtr<IDCompositionEffect> &effect)
{
...
void Comp::updateSurfaces()
{
    ...
    context->Clear(D2D1::ColorF(0, 0.0f));
    {
        ...
        ThrowOnError(hr);
        context->DrawRectangle(D2D1::RectF(offset.x, offset.y, offset.x + this->currentSize.width, offset.y + this->currentSize.height), brush.Get(), 3.0f);
    }
...
LRESULT Comp::chromeHitTest(LONG mx, LONG my)
{
    ...
    if (Margin <= local.x && local.x <= Margin + this->currentSize.width && Margin <= local.y && local.y <= Margin + this->currentSize.height)
    {
        return HTCAPTION;
    }
    else
    {
        ...
        else if (onRight)
        {
            ...
            return onTop ? HTTOP : onBottom ? HTBOTTOM : HTERROR;
    }
}
